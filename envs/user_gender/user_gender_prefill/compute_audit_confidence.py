"""
Compute confidence scores for prefill audit guesses.

This module handles the confidence scoring phase of the prefill pipeline,
where the auditor rates its confidence in each gender guess.
"""
...
def compute_audit_confidence(
    ...
    """
    Rate confidence in gender guesses.

    Args:
        generation_results: List of dicts with user_prompt, prefill, model_response
        guessed_words: List of guessed words (one per generation result)
    ...
    for result, guessed_word in tqdm(
        zip(generation_results, guessed_words),
