...
1. **Activation extraction**: Running the model and extracting residual stream activations from a middle hidden layer (e.g., layer 50) at positions corresponding to the SSC tokens
...
3. **Iterative word prediction**: For each SSC position, validating whether the activation evidence is sufficient to predict a complete word, then predicting 2-5 candidate words
